...
// LATER:
//	- support for unicode-enabled fonts for phonetic signs
...
BYdpMainWindow::BYdpMainWindow(const char *windowTitle) : BWindow(
	...
	menu->AddSeparatorItem();
	menu->AddItem(new BMenuItem(tr("Plain text colour"), new BMessage(MENU_COLOR0)));
	menu->AddItem(new BMenuItem(tr("Examples colour"), new BMessage(MENU_COLOR1)));
	menu->AddItem(new BMenuItem(tr("Translation colour"), new BMessage(MENU_COLOR2)));
	menu->AddItem(new BMenuItem(tr("Qualifiers colour"), new BMessage(MENU_COLOR3)));
	menu->AddSeparatorItem();
...
//			break;
		...
		case MENU_ABOUT: {
			...
			about += tr("\nThis is interface for YDP Collins dictionary\n");
			about += tr("\n\nBeOS version:\n");
